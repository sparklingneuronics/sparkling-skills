...
---
The user wants this **delegated to OpenAI Codex** — run it through Codex; do not answer it yourself. Invoke the `codex` skill and follow its workflow: default to gpt-5.5 and read-only, confirm before any writable sandbox, and keep a topic-aware session so "continue with codex" resumes the right thread. If the request below is empty, delegate what the user is currently working on.
